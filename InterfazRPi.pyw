...
destinatario = "francoarielandrian@gmail.com"
...
isoDia   = 0                                            # En 0 elige de manera automatica el iso entre 100 y 800 de acuerdo de la luz.
isoNoche = 800                                          # Iso maximo permitido.
obturadorDia   = 1000                                   # Velocidad maxima de obturacion en us para congelar movimiento de dia (Para congelar el movimiento obturadorDia < 5000 us)
obturadorNoche = 5000                                   # Velocidad maxima de obturacion en us para congelar movimiento de noche
estadoIluminacion = False                               # False: Iluminacion desactivada - Dia / True: Iluminacion Activada - Noche
CambioEstado      = False                               # False: No hubo cambio de estado / True: Hubo cambio de estado
estadoInicial     = True                                # True: Si el equipo inicia con iluminacion nocturna encendida, que se cambien las configuraciones a modo nocturno.

#Variables para captura de imagen y video
duracion = 1                                            # Duracion de video en segundos
rhi = 1920                                              # Resolucion horizontal de imagen
rvi = 1080                                              # Resolucion vertical   de imagen
rhv = 1920                                              # resolucion horizontal de video
rvv = 1080                                              # resolucion vertical   de video
fps = 30                                                # cuadros por segundo   de video
...
#nuevaLectura = True                                    # Variable local para evitar falsas detecciones 
...
# Configuracion de interrupcion en pin de estado de iluminacion con tiempo minimo entre llamados de 500ms.
GPIO.add_event_detect(pinEstadoIluminacion,GPIO.BOTH, bouncetime = 500)   # Paso de 0 a 1 y de 1 a 0 logico
...
# Configuracion de pines de posicion
...
class Worker1(QThread):                                 # Creamos clase Worker1, heredera de QThread
    ...
    def run(self):                                      # Funcion de ejecucion
        ...
        global pinPIR1, pinPIR2, pinPIR3, pinPIR4, pathAlmacenamiento, posicionPIR, pinDir, pinPaso, nombreArc, path_CyG_Diurnas, path_CyG_Nocturnas, path_CyG_Aux, rhi, rvi, rhv,rvv,fps, GPIO, pinEstadoIluminacion,estadoIluminacion, CambioEstado, estadoInicial, auxGmail, nroCapturas
        ...
        GPIO.add_event_detect(pinPIR1,GPIO.FALLING, bouncetime = 5000)       
        GPIO.add_event_detect(pinPIR2,GPIO.FALLING, bouncetime = 5000)
        GPIO.add_event_detect(pinPIR3,GPIO.FALLING, bouncetime = 5000)
        GPIO.add_event_detect(pinPIR4,GPIO.FALLING, bouncetime = 5000) 
        ...
        while self.ThreadActive:
            ...
            if (estadoInicial or CambioEstado) and estadoIluminacion:                           # EstadoIluminacion = True --> Iluminacion encendida, se hizo de noche.
                cap.configuracionCamara(self.cam,estadoIluminacion, obturadorNoche, isoNoche)
                CambioEstado = False
                estadoInicial = False
                ...
                cap.configuracionCamara(self.cam,estadoIluminacion, obturadorDia, isoDia)
                CambioEstado = False
                estadoInicial = False
                sleep(2)                                # Tiempo necesario para ajustar la ganancia del sensor y los tiempos de exposicion.
            ...
            if ((espacio.used*100)/espacio.total) < 95:
                ...
                if posicionPIR != 0:
                    ...
                    # Si en 2 segundos no se genera una nueva detección, ademas de la actual, se graba el video.
                    while (dt.datetime.now() - start).seconds < 2:
                        
                        if posicionPIR and nuevaLectura:
                    ...
                    window.ui.twConfiguracionSeteada.topLevelItem(1).setText(1,str(cantCapturas+1))
                    window.ui.twConfiguracionSeteada.topLevelItem(2).setText(1,str(cantGrabaciones+1))
                    ...
                    smtp.login(remitente, "yicfizilnxamrhtx")
...
class MainWindow(QMainWindow):
    ...
    def reestablecer(self): 
        global estadoLeyenda, ubicacionEquipo, estadoFechayhora
        if(datos.existeReestablecimiento(pathReestablecimiento)==True):
            ...
            self.ui.cbResImagen.setCurrentIndex(int(auxIndexImg))   
            self.ui.cbResVideo.setCurrentIndex(int(auxIndexVid))
            self.ui.cbNroCapturas.setCurrentIndex(int(auxIndexCapturas))
            self.ui.sbDuracion.setValue(duracionVid)
            self.ui.rbLeyenda.setChecked(estadoLeyenda)
            self.ui.leUbicacion.setText(ubicacionEquipo)

            if not estadoFechayHora:
                self.encender()
    ...
    def encender(self):                                                                             
        global camara, rhi, rvi, resImagen, rhv, rvv, fps, resVideo, duracion, path_CyG_Diurnas, path_CyG_Nocturnas, pathAlmacenamiento, estadoLeyenda, ubicacionEquipo, estadoFechayhora, nroCapturas
