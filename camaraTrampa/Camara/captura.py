...
def configuracionCamara(cam,estadoIluminacion,obturador, iso,fps):
	...
	print('modo sensor: ',cam.sensor_mode)
	print('Velocidad de obturacion:',cam.shutter_speed)
...
def captura(cam,rhi,rvi,nombreArc, estadoLeyenda, cantCapturas,ubicacionEquipo):
	...
	if estadoLeyenda: 
		...
		texto =  ubicacionEquipo + ' - ' + dt.datetime.now().strftime('%d/%m/%Y %H:%M:%S') + ' - Captura Nro.' + str(cantCapturas) 	
...
def grabacion(camara,duracion,rhv,rvv,fps,nombreArc, estadoLeyenda, cantGrabaciones, ubicacionEquipo): # Recprdar que grabacion de video no permite sensor_mode=3, ese se utiliza para el "Modo Noche" que programe.
	...
	if estadoLeyenda:
		...
		camara.start_recording(nombreArc+'.h264', format='h264', profile='high', bitrate=62500000, level='4.2',sei='True')
