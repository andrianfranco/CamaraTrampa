...
class Worker1(QThread):                                 # Creamos clase Worker1, heredera de QThread
    ...
    def run(self):                                      # Funcion de ejecucion
        ...
        global pinPIR1, pinPIR2, pinPIR3, pinPIR4, pathAlmacenamiento, posicionPIR, pinDir, pinPaso, path_CyG_Diurnas, path_CyG_Nocturnas, path_CyG_Aux, rhi, rvi, rhv,rvv,fps, GPIO, pinEstadoIluminacion, nroCapturas, duracion, pinFinCarrera1, ubicacionEquipo, estadoLeyenda
        ...
        while self.ThreadActive:
            if (GPIO.input(pinEstadoIluminacion) and (not(auxUltimoVal) or estadoInicial)):                        # EstadoIluminacion = True --> Iluminacion encendida, se hizo de noche.
                estadoIluminacion = True
                auxUltimoVal = estadoIluminacion
                estadoInicial = False
            ...
            elif(not(GPIO.input(pinEstadoIluminacion)) and (auxUltimoVal or estadoInicial)):                              # EstadoIluminacion = False --> Iluminacion apagada, se hizo de dia.
                estadoIluminacion = False
                auxUltimoVal = estadoIluminacion
                estadoInicial = False
            ...
            if ((espacio.used*100)/espacio.total) < 95:
                ...
                if posicionPIR != 0:
                    ...
                    if auxUltimoVal:
                        cap.configuracionCamara(self.cam,auxUltimoVal, obturadorNoche, isoNoche,fps)
                        sleep(2)
                        print("reconfiguracion de camara")
...
class MainWindow(QMainWindow):
    ...
    def encender(self):                                                                             
        global camara, rhi, rvi, rhv, rvv, fps, duracion, path_CyG_Diurnas, path_CyG_Nocturnas, pathAlmacenamiento, pathReestablecimiento, estadoLeyenda, ubicacionEquipo, estadoFechayhora, nroCapturas
